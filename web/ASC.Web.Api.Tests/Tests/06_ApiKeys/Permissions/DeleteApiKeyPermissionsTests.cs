...
/// <summary>
/// DELETE /api/2.0/keys/{keyId} — permission checks. Anonymous is a plain 401; RoomAdmin, User and
/// Guest deleting the portal owner's own key are all "Bug 81609": the endpoint currently lets a
/// non-owner delete the owner's key instead of returning 403.
/// </summary>
[Trait("Category", "ApiKeys")]
public class DeleteApiKeyPermissionsTests(
    ...
    : ApiKeysTestBase(fixture)
{
    ...
    public async Task DeleteApiKey_Anonymous_ReturnsUnauthorized()
    {
        ...
        var exception = await Assert.ThrowsAsync<ApiException>(
            async () => await _apiKeysApi.DeleteApiKeyAsync(keyId, TestContext.Current.CancellationToken));
    ...
    public async Task DeleteApiKey_NonOwnerRole_CannotDeleteOwnersKey(ApiKeyActor actor)
    {
        // Arrange
        var keyId = await CreateOwnerKeyAsync();
