...
/// <summary>
/// GET /api/2.0/settings/notification/rooms — reads back the caller's own muted-rooms list.
/// </summary>
[Trait("Category", "Settings")]
public class RoomsNotificationSettingsGetTests(
    AspireAppFixture fixture)
    : NotificationsTestBase(fixture)
{
    [Theory]
    [MemberData(nameof(NotificationRoleData.AllRoles), MemberType = typeof(NotificationRoleData))]
    public async Task GetRoomsNotificationSettings_AnyRole_ReturnsThePreviouslyMutedRoom(NotificationActor actor)
    {
        // Arrange
        await AuthenticateAsAsync(actor);
        var roomId = Random.Shared.Next(1, int.MaxValue);

        await _notificationsApi.SetRoomsNotificationStatusAsync(
            new RoomsNotificationsSettingsRequestDto(roomId, true), TestContext.Current.CancellationToken);

        // Act
        var settings = await _notificationsApi.GetRoomsNotificationSettingsWithHttpInfoAsync(TestContext.Current.CancellationToken);
