...
{
    /// <remarks>
    /// Activates the free Docs Connect trial subscription for the current portal, and, once a Docs Connect server is
    ...
    /// already hold a Docs Connect trial, Docs Connect or Docs Connect Dev Pack subscription: the quotas of the current
    ...
    /// a paid plan: an existing paid Docs Connect subscription is moved to Docs Connect Dev Pack by
    ...
    /// <remarks>
    /// Upgrades the paid Docs Connect subscription of the current portal to Docs Connect Dev Pack for the requested
    /// number of users, charging the price difference to the portal wallet and moving the Docs Connect license
    /// to the new product. The portal must hold an active Docs Connect subscription, must not already hold a
    /// Docs Connect Dev Pack one, and its tariff must not be delayed or unpaid: the quotas and the state of the
    ...
    /// fails to perform the switch, in which case nothing is charged and the portal stays on Docs Connect.
    /// Only the Docs Connect to Docs Connect Dev Pack direction is supported: to change the number of users of a
    /// subscription the portal already has, or to schedule a reversion from Docs Connect Dev Pack back to
    /// Docs Connect at the next billing period, use `PUT api/2.0/portal/payment/updatewallet` instead.
    /// </remarks>
    /// <summary>
    ...
    /// <remarks>
    /// Prices the upgrade of the paid Docs Connect subscription of the current portal to Docs Connect Dev Pack for
    /// the requested number of users, without changing the subscription or charging anything. It applies the
    /// same preconditions as the switch itself: the portal must hold an active Docs Connect subscription, must
    /// not already hold a Docs Connect Dev Pack one, and its tariff must not be delayed or unpaid; the quotas and
    ...
    /// <remarks>
    /// Returns the Docs Connect tenant of the current portal: the Docs Connect server assigned to the portal, with its
    /// address, the date the tenant subscription ends and the payment the tenant was created for. A tenant exists
    /// only after a Docs Connect subscription has been granted, by `POST api/2.0/settings/docscloud/trial` or by a
    /// Docs Connect purchase, and only on an installation where the Docs Connect service is configured. The caller must
    ...
    /// `refresh=true` right after a subscription change to read the current state from Docs Connect instead. In the
    /// result, `address` is the absolute URL of the assigned server, `isActive` tells whether `endDate` is still in
    /// the future, and the dates are in UTC. An empty result means the portal has no Docs Connect tenant yet, which is
    /// the normal state before a subscription and not an error, so this is the operation to call to find out whether
    /// Docs Connect is activated at all. The license and server details, the editing settings, the user quota and the
    ...
    /// <remarks>
    /// Returns the Docs Connect license of the current portal, the Docs Connect server serving it, the user limits of
    /// that license and the editor and viewer usage counted against them for the current period. The portal must
    /// have an activated Docs Connect tenant, granted by `POST api/2.0/settings/docscloud/trial` or by a Docs Connect
    ...
    /// installation where the Docs Connect service is configured. The call is read-only, idempotent and cached for a
    /// minute, so pass `refresh=true` right after a subscription change to read the current state from Docs Connect.
    /// In the result, `license.valid` is when the license expires and `license.trial` is reported as `false` once
    /// the portal holds a paid Docs Connect or Docs Connect Dev Pack subscription, even when the license itself still says
    ...
    /// <remarks>
    /// Returns the configuration of the Docs Connect tenant of the current portal: its name, the security secret and
    /// header name, the file size limit and anonymous access switch of the server, the WOPI switch and the IP filter
    /// rules. The portal must have an activated Docs Connect tenant, granted by `POST api/2.0/settings/docscloud/trial`
    /// or by a Docs Connect purchase: an empty result from `GET api/2.0/settings/docscloud/tenant` means there is none
    /// and this call fails with 400. The caller must be a portal administrator allowed to edit the portal settings,
    /// on an installation where the Docs Connect service is configured. The call is read-only, idempotent and cached for
    /// an hour, so pass `refresh=true` to read the current state from Docs Connect; the same values are changed by
    ...
    /// <remarks>
    /// Replaces the configuration of the Docs Connect tenant of the current portal: its name, the security secret and
    /// header name, the file size limit and anonymous access switch of the server, the WOPI switch and the IP filter
    /// rules; it returns the configuration as Docs Connect stored it. The portal must have an activated Docs Connect tenant,
    /// granted by `POST api/2.0/settings/docscloud/trial` or by a Docs Connect purchase: an empty result from
    ...
    /// left out of the request are not sent to Docs Connect at all, while a section that is present is sent with all of
    /// its fields, so a field left unset inside it goes out as `0`, `false` or empty. The caller must be a portal
    /// administrator allowed to edit the portal settings, on an installation where the Docs Connect service is
    ...
    /// Docs Connect. It changes these settings only, never the subscription, the user quota or the license.
    /// </remarks>
    /// <summary>
    ...
    /// <remarks>
    /// Returns the Docs Connect user quota of the current portal: the users who currently count as Docs Connect editors and
    /// the users who count as viewers, each with the identifier Docs Connect knows them by and the date their quota entry
    /// expires. The portal must have an activated Docs Connect tenant, granted by `POST api/2.0/settings/docscloud/trial`
    /// or by a Docs Connect purchase: an empty result from `GET api/2.0/settings/docscloud/tenant` means there is none
    /// and this call fails with 400. The caller must be a portal administrator allowed to edit the portal settings,
    /// on an installation where the Docs Connect service is configured. The call is read-only, idempotent and cached for
    /// a minute, so pass `refresh=true` to read the current state from Docs Connect. In the result, `users` holds the
    /// editor entries and `usersView` the viewer entries, both unordered; `userId` is the DocSpace user ID for a
    /// portal member and an identifier of Docs Connect's own for anyone else; `expire` is the date and time the entry
    ...
    /// <remarks>
    /// Queues a background job that renders the current Docs Connect user quota of the portal into an "xlsx" file and
    ...
    /// portal should have an activated Docs Connect tenant: this call does not check that, and without a tenant the job
    ...
    /// <remarks>
    /// Returns the state of the Docs Connect user quota report that the current user started with
    ...
    /// <path>api/2.0/settings/docscloud/tenant/quota/report</path>
    [Tags("Settings / DocsCloud")]
    [SwaggerResponse(200, "The state of the Docs Connect quota report job of the caller, or an empty result if there is no such job", typeof(DocumentBuilderTaskDto))]
    [SwaggerResponse(403, "The caller is not allowed to edit the portal settings")]
    ...
    /// <remarks>
    /// Cancels the Docs Connect user quota report that the current user started with
    ...
    /// <remarks>
    /// Returns the Docs Connect usage of the current portal: the number of users who have been active in Docs Connect in
    /// the current period, and the moment that period is counted from. The portal must have an activated Docs Connect
    /// tenant, granted by `POST api/2.0/settings/docscloud/trial` or by a Docs Connect purchase: an empty result from
    /// `GET api/2.0/settings/docscloud/tenant` means there is none and this call fails with 400. The caller must be a
    /// portal administrator allowed to edit the portal settings, on an installation where the Docs Connect service is
    /// configured. The call is read-only, idempotent and cached for a minute, so pass `refresh=true` to read the
    /// current state from Docs Connect. In the result, `activeCount` counts the users seen since `since`, which is in
