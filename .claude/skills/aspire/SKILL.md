...
This repository uses Aspire to orchestrate its distributed application.

## This project's AppHost

The AppHost is **not** in the repo root or a default location — it lives in **`common/ASC.AppHost`**:

- Project file: `common/ASC.AppHost/ASC.AppHost.csproj`
- Resource graph is defined in classic C# style across **`common/ASC.AppHost/Program.cs`** and the `common/ASC.AppHost/Configuration/` folder (`ProjectConfigurator.cs`, `ConnectionStringManager.cs`, `NginxConfiguration.cs`, etc.) — **not** in a single-file `apphost.cs`/`apphost.ts`.

Because the AppHost is in a non-default subdirectory, either run Aspire CLI commands from within `common/ASC.AppHost`, or pass `--apphost common/ASC.AppHost/ASC.AppHost.csproj` (paths relative to the repo root) to every command. For example:

```bash
aspire start --apphost common/ASC.AppHost/ASC.AppHost.csproj
aspire describe --apphost common/ASC.AppHost/ASC.AppHost.csproj
```

When this doc below refers to "AppHost code" or `apphost.cs`, it means **`common/ASC.AppHost/Program.cs`** and the files under `common/ASC.AppHost/Configuration/`.
...
|---|---|---|
| AppHost project (`common/ASC.AppHost/Program.cs` or files in `common/ASC.AppHost/Configuration/`) | `aspire start` | Resource graph changed; full restart required |
