...
public static class FormSchemaFormatter
{
    ...
    /// <summary>One column per line, as a bullet list.</summary>
    public static string FormatColumnList(IEnumerable<DbColumnDefinition> columns, int maxEnumValues)
    {
        return string.Join("\n", columns.Select(c => $"- {FormatColumn(c, maxEnumValues)}"));
    }
}
