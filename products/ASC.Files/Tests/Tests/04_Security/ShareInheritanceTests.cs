    ...
    : BaseTest(fixture)
{
    ...
    public async Task CheckExternalRightsFile_Documents_ReturnFormFilling()
    {
        ...
        fileInfo.AvailableShareRights.ExternalLink.Should().HaveCount(5);
        fileInfo.AvailableShareRights.ExternalLink.Should().Contain(nameof(FileShare.Editing), nameof(FileShare.FillForms), nameof(FileShare.Comment), nameof(FileShare.Read), nameof(FileShare.None));
    }
