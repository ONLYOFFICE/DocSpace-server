...
/// <summary>
/// <c>DELETE /files/file/{fileId}</c> — positive/functional coverage (move to Trash, immediate
/// delete, the async operation's shape) and locking/edit-conflict edge cases. Access control lives
/// in <see cref="FileDeletePermissionsTests"/>.
/// </summary>
[Trait("Category", "CRUD")]
[Trait("Feature", "Files")]
    ...
    : BaseTest(fixture)
{
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task DeleteFile_MyDocuments_Owner_OperationHasExpectedShape(bool immediately)
    {
        // Arrange
        ...
        operation.Progress.Should().Be(100);
        operation.Processed.Should().Be("1");
        operation.Error.Should().BeNullOrEmpty();
    }

    [Fact]
    public async Task DeleteFile_InRoom_Owner_DeletesImmediately()
    {
        // Arrange
        await _filesClient.Authenticate(Owner);
        var room = await CreateCustomRoom("Autotest Delete Room File Immediate");
