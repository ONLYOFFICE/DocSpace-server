    ...
    : BaseTest(fixture)
{
    ...
    [Fact]
    public async Task CreateFolder_InRoom_Owner_ReturnsOk()
    {
        var room = await CreateCustomRoom("Autotest Room For Folder Creation");

        var createdFolder = await CreateFolder("Autotest Folder", room.Id);

        createdFolder.Should().NotBeNull();
        createdFolder.Title.Should().Be("Autotest Folder");
        createdFolder.ParentId.Should().Be(room.Id);
        createdFolder.Id.Should().BeGreaterThan(0);
    }
