...
/// <summary>
/// <c>PUT /files/rooms/{id}/unpin</c> can unpin every non-AI room type.
/// </summary>
[Trait("Category", "Rooms")]
public class RoomUnpinRoomTypesTests(
    ...
    : RoomPinTestsBase(fixture)
{
    ...
    public async Task UnpinRoom_OfEachType_Unpins(RoomType roomType)
    {
        // Arrange
        await _filesClient.Authenticate(Owner);
        var room = await CreateRoomOfType(roomType, $"Autotest Unpin {roomType}");
        ...
        var (_, row, _, _) = await FindRoomRow(room.Id, SearchAreaFor(roomType));
