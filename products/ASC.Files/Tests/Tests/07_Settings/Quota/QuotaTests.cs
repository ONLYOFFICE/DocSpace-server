...
/// <summary>
/// Room-quota lifecycle (set/reset) and the soft/hard exhaustion limits enforced on file
/// creation and chunked upload. The plain access-control matrix, the <c>isCustomQuota</c> flag,
/// empty payloads, archived rooms and the feature toggle are covered by the sibling classes in
/// this folder instead of being duplicated here.
/// </summary>
    ...
    : QuotaTestBase(fixture)
{
    ...
    public async Task ResetRoomQuota_ValidRoomIds_ResetsQuotaSuccessfully()
    {
        ...
        await EnableRoomQuota(defaultQuotaLimit);
