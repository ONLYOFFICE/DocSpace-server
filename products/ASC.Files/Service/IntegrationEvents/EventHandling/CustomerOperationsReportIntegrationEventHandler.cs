    ...
    public async Task Handle(CustomerOperationsReportIntegrationEvent @event)
    {
        ...
        using (logger.BeginScope(new[] { new KeyValuePair<string, object>("integrationEventContext", $"{@event.Id}-{Program.AppName}") }))
        {
            ...
            try
            {
                ...
                task.Init(@event.BaseUri, @event.TenantId, @event.CreateBy, new CustomerOperationsReportTaskData(@event.Headers, @event.StartDate, @event.EndDate, @event.Credit, @event.Withdrawal));
