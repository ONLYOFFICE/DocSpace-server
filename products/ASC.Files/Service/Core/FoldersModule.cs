...
public class FoldersModule(TenantManager tenantManager,
    ...
    : FeedModule(tenantManager, webItemSecurity)
{
    ...
    public override async Task<IEnumerable<Tuple<Feed.Aggregator.Feed, object>>> GetFeeds(FeedFilter filter)
    {
        ...
        return folders.Select(f => new Tuple<Feed.Aggregator.Feed, object>(ToFeed(f, parentFolders.Find(r => r.Id.Equals(f.Folder.ParentId)),
    ...
    private Feed.Aggregator.Feed ToFeed(FolderWithShare folderWithSecurity, Folder<int> parentFolder, int roomId)
    {
        ...
        var folderCreatedUtc = tenantUtil.DateTimeToUtc(folder.CreateOn);
