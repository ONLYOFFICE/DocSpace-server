...
[Singleton]
public class CleanupMarkedEntriesService(
    IServiceScopeFactory scopeFactory,
    IConfiguration configuration,
    ILogger<CleanupMarkedEntriesService> logger)
    : ActivePassiveBackgroundService<CleanupMarkedEntriesService>(logger, scopeFactory)
{
    private readonly IServiceScopeFactory _serviceScopeFactory = scopeFactory;

    protected override TimeSpan ExecuteTaskPeriod { get; set; } = TimeSpan.Parse(configuration.GetValue<string>("files:cleanupMarkedEntries:period") ?? "1:0:0");

    protected override async Task ExecuteTaskAsync(CancellationToken stoppingToken)
    {
        logger.TraceCleanupMarkedEntriesProcedureStart();

        await DeleteMarkedEntries(stoppingToken);

        logger.TraceCleanupMarkedEntriesProcedureFinish();
    }

    private async Task DeleteMarkedEntries(CancellationToken cancellationToken)
    {
