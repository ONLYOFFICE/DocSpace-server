      ...
      IIntegrationEventHandler<CreateRoomFromTemplateIntegrationEvent>
{
    ...
    {
        await worker.StartCreateRoomAsync(@event.TenantId, @event.CreateBy, @event.TemplateId, @event.Title, @event.Logo, @event.CopyLogo, @event.Tags, @event.Cover, @event.Color, @event.Quota, true, @event.TaskId);
    }
}
