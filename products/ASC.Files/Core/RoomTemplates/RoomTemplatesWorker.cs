    ...
    IDistributedLockProvider distributedLockProvider)
{
    ...
    private readonly DistributedTaskQueue<CreateRoomTemplateOperation> _templateQueue = queueFactory.CreateQueue<CreateRoomTemplateOperation>(CUSTOM_DISTRIBUTED_TASK_QUEUE_NAME);
    private readonly DistributedTaskQueue<CreateRoomFromTemplateOperation> _roomQueue = queueFactory.CreateQueue<CreateRoomFromTemplateOperation>(CUSTOM_DISTRIBUTED_TASK_QUEUE_NAME);
        ...
        string taskId = null)
    {
        await using (await distributedLockProvider.TryAcquireLockAsync(LockKey))
        {
            ...
            if (item == null || (enqueueTask && item.Id == taskId && item.Status == DistributedTaskStatus.Created))
            {
                ...
                item.Init(tenantId, userId, roomId, title, emails, logo, CopyLogo, tags, groups, cover, color);
        ...
        string taskId = null)
    {
        await using (await distributedLockProvider.TryAcquireLockAsync(LockKey))
        {
            ...
            if (item == null || (enqueueTask && item.Id == taskId && item.Status == DistributedTaskStatus.Created))
            {
                ...
                item.Init(tenantId, userId, templateId, title, logo, copyLogo, tags, cover, color);
