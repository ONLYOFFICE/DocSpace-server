...
{
    private readonly TemplateGallerySettings _configuration = configuration.GetSection("files:template-gallery").Get<TemplateGallerySettings>();
    ...
    public async Task<Stream> Get(int id, string ext)
    {
        try
        {
            ...
            using var response = await httpClient.GetAsync($"{_configuration.Upload.Domain.TrimEnd('/')}/{_configuration.Upload.Dashboard.Trim('/')}/oforms/{id}?populate[file_oform][fields]=url&populate[file_oform][fields]=name&populate[file_oform][fields]=ext&populate[file_oform][filters][url][$endsWith]={ext}");
