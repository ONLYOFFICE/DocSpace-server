...
{
    Task<List<FileOperationResult>> GetOperationResults();
...
{
    protected readonly IPublishEndpoint _bus = bus;
    ...
    {
        return await _fileOperationsManagerHolder.GetOperationResults(await GetUserIdAsync());
    }
...
public class FileMarkAsReadOperationsManager(
    ...
    ExternalShare externalShare,
    IServiceProvider serviceProvider) : FileOperationsManager<FileMarkAsReadOperation>(httpContextAccessor, bus, authContext, fileOperationsManagerHolder, externalShare, serviceProvider)
{
    public async Task Publish(IEnumerable<JsonElement> folderIds, IEnumerable<JsonElement> fileIds)
    {
        ...
        await _bus.Publish(new MarkAsReadIntegrationEvent(_authContext.CurrentAccount.ID, tenantId)
        {
            TaskId = taskId,
            Data = data,
...
public class FileDownloadOperationsManager(
    ...
    ExternalShare externalShare,
    IServiceProvider serviceProvider) : FileOperationsManager<FileDownloadOperation>(httpContextAccessor, bus, authContext, fileOperationsManagerHolder, externalShare, serviceProvider)
    {
    public async Task Publish(IEnumerable<JsonElement> folders, IEnumerable<FilesDownloadOperationItem<JsonElement>> files, string baseUri)
    {
        ...
        await _bus.Publish(new BulkDownloadIntegrationEvent(await GetUserIdAsync(), tenantId)
        {
            TaskId = taskId,
            Data = data,
            ThirdPartyData = thirdPartyData
...
public class FileMoveCopyOperationsManager(
    ...
    ExternalShare externalShare,
    IServiceProvider serviceProvider) : FileOperationsManager<FileMoveCopyOperation>(httpContextAccessor, bus, authContext, fileOperationsManagerHolder, externalShare, serviceProvider)
{
    ...
    {        
        ...
        await _bus.Publish(new MoveOrCopyIntegrationEvent(_authContext.CurrentAccount.ID, tenantId)
        {
            TaskId = taskId,
            Data = data,
            ThirdPartyData = thirdPartyData
...
public class FileDuplicateOperationsManager(
    ...
    ExternalShare externalShare,
    IServiceProvider serviceProvider) : FileOperationsManager<FileDuplicateOperation>(httpContextAccessor, bus, authContext, fileOperationsManagerHolder, externalShare, serviceProvider)
{
    public async Task Publish(
        IEnumerable<JsonElement> folderIds,
        IEnumerable<JsonElement> fileIds)
    {        
        ...
        await _bus.Publish(new DuplicateIntegrationEvent(_authContext.CurrentAccount.ID, tenantId)
        {
            TaskId = taskId,
            Data = data,
            ThirdPartyData = thirdPartyData
...
public class FileDeleteOperationsManager(
    ...
    ExternalShare externalShare,
    IServiceProvider serviceProvider) : FileOperationsManager<FileDeleteOperation>(httpContextAccessor, bus, authContext, fileOperationsManagerHolder, externalShare, serviceProvider)
{
    ...
    {        
        ...
        if (isEmptyTrash)
        {
            await _bus.Publish(new EmptyTrashIntegrationEvent(_authContext.CurrentAccount.ID, tenantId)
            {
                TaskId = taskId, 
                Data = data,
                ThirdPartyData = thirdPartyData
        ...
        {
            await _bus.Publish(new DeleteIntegrationEvent(_authContext.CurrentAccount.ID, tenantId)
            {
                TaskId = taskId, 
                Data = data,
                ThirdPartyData = thirdPartyData
