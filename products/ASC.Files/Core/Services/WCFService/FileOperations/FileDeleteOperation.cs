...
record FileDeleteOperationData<T>(
    ...
    : FileOperationData<T>(Folders, Files, TenantId, Headers, HoldResult, HiddenOperation: IsEmptyTrash && Immediately);
...
class FileDeleteOperation<T> : FileOperation<FileDeleteOperationData<T>, T>
{
    ...
    protected override async Task DoJob(IServiceScope serviceScope)
    {
        ...
        if (_isEmptyTrash)
        {
            if (_immediately)
            {
                await DeleteFilesAsync(Files, serviceScope, false, false);
                await DeleteFoldersAsync(Folders, serviceScope, false, false);
            }
            else
            {
                ...
                await MarkFolderContentAsRemovedAsync(socketManager, FolderDao, FileDao, trash as Folder<T>);

                await filesMessageService.SendAsync(MessageAction.TrashEmptied, trash, Headers);

                _ = fileOperationsManager.PublishDelete(Folders, Files, false, false, true, true);
            }
        }
        else
        {
    ...
    private async Task MarkFolderContentAsRemovedAsync(SocketManager socketManager, IFolderDao<T> folderDao, IFileDao<T> fileDao, Folder<T> folder)
    {
        ...
        await foreach (var subfolder in subfolders)
        {
            ...
            await MarkFolderContentAsRemovedAsync(socketManager, folderDao, fileDao, subfolder);
        }
    }
            ...
            else
            {
                ...
                if (folder.ProviderEntry && folder.Id.Equals(folder.RootId))
                {
                    if (ProviderDao != null)
                    {
                        ...
                        {
                            await filesMessageService.SendAsync(MessageAction.ThirdPartyDeleted, folder, Headers, folder.Id.ToString(), folder.ProviderKey);
                        }
                    }
