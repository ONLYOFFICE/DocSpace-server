...
[Singleton]
public class FormRecoveryTaskManager(IDistributedTaskQueueFactory queueFactory) : IDisposable
{
    private const string TaskIdPrefix = "FormRecoveryTask";

    private readonly SemaphoreSlim _semaphore = new(1);
    private readonly SemaphoreSlim _busyCheckLock = new(1, 1);
    private readonly DistributedTaskQueue<FormRecoveryTask> _queue = queueFactory.CreateQueue<FormRecoveryTask>();

    public static string GetTaskId(int tenantId, int roomId) => $"{TaskIdPrefix}_{tenantId}_{roomId}";

    public async Task<FormRecoveryTask> GetTask(int tenantId, int roomId)
    {
    ...
    public async Task<bool> IsTooBusy()
    {
        ...
        try
        {
            var instanceTasks = await _queue.GetAllTasks(DistributedTaskQueue<FormRecoveryTask>.INSTANCE_ID);
    ...
    public Task<FormRecoveryTask> EnqueueTaskAsync(FormRecoveryTask task) => StartTaskCoreAsync(task, enqueue: true);

    private async Task<FormRecoveryTask> StartTaskCoreAsync(FormRecoveryTask newTask, bool enqueue)
    {
        try
        {
