        ...
        distributedLockProvider), ITagDao<T>
{
    ...
    public async Task<TagInfo> UpdateTagInfoAsync(TagInfo tagInfo)
    {
        ...
        var rooms = await folderDao.GetRoomsAsync([await globalFolder.GetFolderVirtualRoomsAsync(daoFactory)], null, new List<string> { tagInfo.Name }, Guid.Empty, null, false, false, false, ProviderFilter.None, Guid.Empty, null, QuotaFilter.All).ToListAsync();
        var thirdPartyRooms = await folderThirdPartyDao.GetProviderBasedRoomsAsync(SearchArea.Active, null, new List<string> { tagInfo.Name }, Guid.Empty, null, false, false, ProviderFilter.None, Guid.Empty, null).ToListAsync();
        ...
        await using (await _distributedLockProvider.TryAcquireLockAsync(GetLockKey(tenantId), TimeSpan.FromMinutes(5)))
        {
            ...
            await strategy.ExecuteAsync(async () =>
            {
                ...
                foreach (var t in tags)
                {
                    ...
                    var id = await internalFilesDbContext.FirstTagIdAsync(tenantId, t.Owner, t.Name, t.Type);
    ...
    private async Task<Tag> SaveTagAsync(FilesDbContext filesDbContext, int tenantId, Tag t, Dictionary<string, int> cacheTagId, DateTime createOn, Guid createdBy = default)
    {
        ...
        {
            id = await filesDbContext.FirstTagIdAsync(tenantId, t.Owner, t.Name, t.Type);
    ...
    private async ValueTask RemoveTagInDbAsync(Tag tag)
    {
        ...
        if (id != 0)
        {
            ...
            var rooms = await folderDao.GetRoomsAsync([await globalFolder.GetFolderVirtualRoomsAsync(daoFactory)], null, new List<string> { tag.Name }, Guid.Empty, null, false, false, false, ProviderFilter.None, Guid.Empty, null, QuotaFilter.All).ToListAsync();
            var thirdPartyRooms = await folderThirdPartyDao.GetProviderBasedRoomsAsync(SearchArea.Active, null, new List<string> { tag.Name }, Guid.Empty, null, false, false, ProviderFilter.None, Guid.Empty, null).ToListAsync();
