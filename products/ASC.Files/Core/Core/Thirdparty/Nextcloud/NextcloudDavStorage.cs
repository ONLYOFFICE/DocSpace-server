    ...
    : IThirdPartyStorage<WebDavEntry, WebDavEntry, WebDavEntry>, IDisposable
{
    ...
    public void Open(AuthData authData)
    {
        ...
        authData.Url = new Uri(nextcloudLoginProvider.BaseUri, "/remote.php/dav/files/admin").ToString(); // ToDo: pull actual username instead of 'admin'
        ...
        httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authData.Token.AccessToken); // ToDo: token won't refresh, move it to SendAsync() and use AccessToken property
