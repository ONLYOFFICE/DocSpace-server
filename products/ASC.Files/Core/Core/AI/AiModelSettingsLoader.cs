    ...
    IDbContextFactory<FilesDbContext> dbContextFactory)
{
        ...
        IFolder currentFolder)
    {
        ...
        if (currentFolder.FolderType is FolderType.AiAgents)
        {
            ...
            {
                if (entry is Folder<int> { FolderType: FolderType.AiRoom, ChatSettings.ProviderId: > 0 } folder)
                {
                    providerIds.Add(folder.ChatSettings.ProviderId);
                }
            }
        }
        else if (currentFolder is Folder<int> { ChatSettings.ProviderId: > 0 } current)
        {
            providerIds = [current.ChatSettings.ProviderId];
        }
