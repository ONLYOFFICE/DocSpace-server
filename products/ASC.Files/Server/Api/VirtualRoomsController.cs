        ...
        roomService)
{
    ...
    public async Task<FolderDto<int>> CreateRoomAsync(CreateRoomRequestDto inDto)
    {
        ...
        var room = await _roomService.CreateRoomAsync(inDto.Title, inDto.RoomType, inDto.Private, inDto.Indexing, inDto.Share, inDto.Quota, lifetime, inDto.DenyDownload, inDto.Watermark, inDto.Color, inDto.Cover, inDto.Tags, inDto.Logo);
        ...
        roomService)
{
    ...
    {
        var room = await _roomService.CreateThirdPartyRoomAsync(inDto.Room.Title, inDto.Room.RoomType, inDto.Id, inDto.Room.Private, inDto.Room.Indexing, inDto.Room.CreateAsNewFolder, inDto.Room.DenyDownload, inDto.Room.Color, inDto.Room.Cover, inDto.Room.Tags, inDto.Room.Logo);
    ...
    : ApiControllerBase(folderDtoHelper, fileDtoHelper)
{
    ...
    protected readonly RoomService _roomService = roomService;
    ...
    {
        var folder = await folderService.GetFolderAsync(inDto.Id).NotFoundIfNull("Folder not found");
    ...
    {
        var room = await _roomService.UpdateRoomAsync(inDto.Id, inDto.UpdateRoom);
    ...
    public async IAsyncEnumerable<FolderDto<int>> UpdateRoomsQuotaAsync(UpdateRoomsQuotaRequestDto<T> inDto)
    {
        ...
        {
            var room = await _roomService.FolderQuotaChangeAsync(roomId, inDto.Quota);
    ...
    public async IAsyncEnumerable<FolderDto<int>> ResetRoomQuotaAsync(UpdateRoomsRoomIdsRequestDto<T> inDto)
    {
        ...
        foreach (var roomId in folderIntIds)
        {
            var room = await _roomService.FolderQuotaChangeAsync(roomId, -2);
            folderTitles.Add(room.Title);
    ...
    {
        var room = await folderService.GetFolderAsync(inDto.Id);
    ...
    public async Task<RoomSecurityDto> SetRoomSecurityAsync(RoomInvitationRequestDto<T> inDto)
    {
        ...
        var room = await folderService.GetFolderAsync(inDto.Id).NotFoundIfNull("Folder not found");
        ...
        foreach(var invitation in inDto.RoomInvitation.Invitations)
        {
            ...
            {
                if (await _roomService.ShouldPreventUserDeletion(room, invitation.Id))
                {
        ...
        result.Members = await _roomService.GetRoomSharedInfoAsync(inDto.Id, inDto.RoomInvitation.Invitations.Select(s => s.Id))
    ...
    public async IAsyncEnumerable<FileShareDto> GetRoomSecurityInfoAsync(RoomSecurityInfoRequestDto<T> inDto)
    {
        ...
        var totalCountTask = await sharingService.GetPureSharesCountAsync(inDto.Id, FileEntryType.Folder, inDto.FilterType, text);
        apiContext.SetCount(Math.Min(totalCountTask - offset, count)).SetTotalCount(totalCountTask);

        await foreach (var ace in sharingService.GetPureSharesAsync(inDto.Id, FileEntryType.Folder, inDto.FilterType, text, offset, count))
        {
    ...
    public async Task<FileShareDto> SetLinkAsync(RoomLinkRequestDto<T> inDto)
    {
        ...
        {
            LinkType.Invitation => await sharingService.SetInvitationLinkAsync(inDto.Id, inDto.RoomLink.LinkId, inDto.RoomLink.Title, inDto.RoomLink.Access),
            LinkType.External => await sharingService.SetExternalLinkAsync(inDto.Id, FileEntryType.Folder, inDto.RoomLink.LinkId, inDto.RoomLink.Title,
    ...
    public async IAsyncEnumerable<FileShareDto> GetRoomLinksAsync(GetRoomLinksRequestDto<T> inDto)
    {
        ...
        await foreach (var ace in sharingService.GetPureSharesAsync(inDto.Id, FileEntryType.Folder, filterType, null, 0, 100))
        {
            counter++;
    ...
    {
        var room = await _roomService.SetPinnedStatusAsync(inDto.Id, true);
    ...
    {
        var room = await _roomService.SetPinnedStatusAsync(inDto.Id, false);
    ...
    {
        await _roomService.ResendEmailInvitationsAsync(inDto.Id, inDto.UserInvitation.UsersIds, inDto.UserInvitation.ResendAll);
    }
    ...
    {
        var newItems = await _roomService.GetNewRoomFilesAsync(inDto.Id);
    ...
    : ApiControllerBase(folderDtoHelper, fileDtoHelper)
{
    ...
    public async Task<FolderContentDto<int>> GetRoomsFolderAsync(RoomContentRequestDto inDto)
    {
        ...
        var content = await folderService.GetFolderItemsAsync(
            parentId,
            startIndex,
            count,
            filter,
    ...
    {
        var newItems = await roomService.GetNewRoomFilesAsync();
