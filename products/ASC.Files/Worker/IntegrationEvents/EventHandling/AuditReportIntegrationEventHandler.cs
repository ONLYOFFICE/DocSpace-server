...
[Scope]
public class AuditReportIntegrationEventHandler(
    ILogger<AuditReportIntegrationEventHandler> logger,
    CommonLinkUtility commonLinkUtility,
    TenantManager tenantManager,
    DocumentBuilderTaskManager<AuditReportTask, int, AuditReportTaskData> documentBuilderTaskManager,
    IServiceProvider serviceProvider)
    : IIntegrationEventHandler<AuditReportIntegrationEvent>
{

    public async Task Handle(AuditReportIntegrationEvent @event)
    {
        ...
        using (logger.BeginScope(new[] { new KeyValuePair<string, object>("integrationEventContext", $"{@event.Id}-{Program.AppName}") }))
        {
            ...
            try
            {
                ...
                var data = new AuditReportTaskData(
                    @event.Kind,
                    @event.Format,
                    @event.From,
                    @event.To,
