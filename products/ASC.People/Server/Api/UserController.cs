...
public class UserController(ICache cache,
    ...
    {
        var users = GetByFilterAsync(employeeStatus, groupId, activationStatus, employeeType, employeeTypes, isAdministrator, payments, accountLoginType, quotaFilter);
    ...
    public async IAsyncEnumerable<EmployeeFullDto> ResetUsersQuota(UpdateMembersQuotaRequestDto inDto)
    {
        ...
        foreach (var user in users)
        {
            ...
            var quotaUserSettings = await settingsManager.LoadAsync<TenantUserQuotaSettings>();
            var userQuotaLimit = userQuotaData.UserQuota == userQuotaData.GetDefault().UserQuota ? quotaUserSettings.DefaultQuota : userQuotaData.UserQuota;
            _ = quotaSocketManager.ChangeCustomQuotaUsedValueAsync(tenant.Id, customQuota.GetFeature<UserCustomQuotaFeature>().Name, quotaUserSettings.EnableQuota, userUsedSpace.ToString(), userQuotaLimit.ToString(), new List<Guid>() { user.Id });
