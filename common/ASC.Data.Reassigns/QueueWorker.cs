    ...
    where T : DistributedTaskProgress
{
    ...
    /// <summary>
    /// The portal address of the request that starts the task. The task itself runs without an
    /// HTTP context, where <see cref="BaseCommonLinkUtility.ServerRootPath"/> falls back to the
    /// tenant domain — plain "localhost" on a standalone portal without a base domain, which is
    /// unreachable from a split-container deployment.
    /// </summary>
    protected string GetBaseUri(BaseCommonLinkUtility baseCommonLinkUtility)
    {
        return httpContextAccessor.HttpContext?.Request != null ? baseCommonLinkUtility.ServerRootPath : null;
    }
    ...
    : QueueWorker<ReassignProgressItem>(httpContextAccessor, serviceProvider, queueFactory, distributedLockProvider)
{
    public async Task<ReassignProgressItem> StartAsync(int tenantId, Guid fromUserId, Guid toUserId, Guid currentUserId, bool notify, bool deleteProfile)
    {
        ...
        result.Init(_httpHeaders, GetBaseUri(baseCommonLinkUtility), tenantId, fromUserId, toUserId, currentUserId, notify, deleteProfile);
    ...
    : QueueWorker<RemoveProgressItem>(httpContextAccessor, serviceProvider, queueFactory, distributedLockProvider)
{
    public async Task<RemoveProgressItem> StartAsync(int tenantId, UserInfo user, Guid currentUserId, bool notify, bool deleteProfile, bool isGuest)
    {
        ...
        result.Init(_httpHeaders, GetBaseUri(baseCommonLinkUtility), tenantId, user, currentUserId, notify, deleteProfile, isGuest);
