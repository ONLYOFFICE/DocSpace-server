...
/**
 * AscAdminCookieAuthenticationFilter is a Spring Security filter that processes requests to
 * validate admin users based on ASC authentication cookies.
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class AscAdminCookieAuthenticationFilter extends OncePerRequestFilter {
  @Value("${web.api}")
  ...
  /**
   * Filters the request to validate admin users.
   *
  ...
  protected void doFilterInternal(
    ...
    try {
      ...
      var attribute = request.getAttribute("me");
      if (attribute instanceof AscPersonResponse me && !me.isAdmin()) {
  ...
  private boolean isExcludedPath(String path) {
    return Pattern.matches(String.format("%s/clients/.*/info", webApi), path)
        || Pattern.matches(String.format("%s/clients/consents", webApi), path)
        || Pattern.matches(String.format("%s/clients/.*/revoke", webApi), path)
        || Pattern.matches(String.format("%s/oauth/info", webApi), path)
