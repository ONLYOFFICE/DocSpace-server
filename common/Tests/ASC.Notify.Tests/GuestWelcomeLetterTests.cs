...
/// <summary>
/// The welcome letter as a guest receives it. The four guest actions render the very same
/// <c>user_welcome_v1</c> template as the five user ones (see <see cref="UserWelcomeLetterTests"/>), so
/// what this class adds is the other footer flavour: Opensource asks for <c>opensource</c> where SaaS
/// asks for <c>social</c>.
/// </summary>
public class GuestWelcomeLetterTests : LetterTestBase
{
    ...
    /// <summary>Names the preview after the action, since the template itself is shared.</summary>
    protected override string LetterId => "opensource_guest_welcome_v1";
    ...
    /// <summary>The Opensource footer, the one the user-letter test does not exercise.</summary>
    protected override string Footer => "opensource";
