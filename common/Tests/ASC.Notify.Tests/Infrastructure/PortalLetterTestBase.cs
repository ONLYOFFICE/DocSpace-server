...
/// <summary>
/// Renders a letter from the tags the production action itself produces, against a real portal.
///
/// The difference from <see cref="LetterTestBase{TAction}"/> is where the tag values come from. There, a
/// test restates them — the orange button, the footer flavour, the top image, whether the signature is a
/// table row — which is a copy of the action's <c>Init</c> that can fall out of step with it silently:
/// the test stays green while rendering a letter the sending code no longer produces. Here the test only
/// says how <c>Init</c> is called, and the tags are read back off the action.
///
...
/// without a portal. <see cref="LetterTestBase{TAction}"/> remains for the letters not migrated yet.
/// </summary>
/// <typeparam name="TAction">The notify action that sends this letter in production.</typeparam>
public abstract class PortalLetterTestBase<TAction> where TAction : NotifyAction
{
    ...
    /// html-check scores it against the caniemail support matrix. See
    /// <see cref="LetterTestBase{TAction}.MinimumHtmlSupport"/> for why the floor sits where it does.
    /// </summary>
    ...
    private async Task<(TAction Action, IPattern Pattern, List<ITagValue> Tags)> PrepareAsync(LetterScope scope)
    {
        ...
        var tags = BuildCommonTags(scope);
    ...
    ///
    /// Values still come from <see cref="LetterEnvironment"/> and the portal rather than from the real
    /// transfer step. The step that closes that gap is to run <c>NotifyTransferRequest</c> itself and
    /// delete this method; until then two things it does are knowingly not reproduced — the
    /// <c>LetterLogo</c> attachment it adds to letters without a top image, and its removal of
    /// <c>TopGif</c> from a white-labelled portal.
    /// </summary>
    private static List<ITagValue> BuildCommonTags(LetterScope scope)
    {
        var logoText = LetterEnvironment.LogoText;

        return
        [
            new TagValue(CommonTags.AuthorID, scope.Tenant.OwnerId),
            new TagValue(CommonTags.AuthorName, scope.Recipient.FirstName),
            new TagValue(CommonTags.AuthorUrl, $"{scope.PortalUrl}/accounts/view/@self"),
            ...
            new TagValue(CommonTags.ProfileUrl, $"{scope.PortalUrl}/accounts/view/@self"),
