...
export const webSearchController = {
  getActiveConfig: asyncHandler(async (_req, res) => {
    const config = await engine.getActiveConfig();
    res.json(config);
  }),

  isConfigured: asyncHandler(async (_req, res) => {
    const value = await engine.isConfigured();
    res.json({ value });
  }),
