        ...
        using (logger.BeginScope(new[] { new KeyValuePair<string, object>("integrationEventContext", $"{@event.Id}-{Program.AppName}") }))
        {
            ...
            if (@event.IsScheduled)
            {
                await backupWorker.StartScheduledBackupAsync(new BackupSchedule
                {
                    ...
                    Dump = @event.Dump
                }, @event.BillingSessionId);
            }
            else
            {
                await backupService.StartBackupAsync(@event.StorageType, @event.StorageParams, @event.ServerBaseUri, @event.Dump, true, @event.TaskId, @event.BillingSessionId);
            }
        }
    }
}