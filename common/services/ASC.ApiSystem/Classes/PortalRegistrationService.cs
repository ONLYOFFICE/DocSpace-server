    ...
    ConsumerFactory consumerFactory)
{
    ...
    public async Task<(LoginProfile Profile, bool AutoGeneratedEmail)> ProcessThirdPartyProfileAsync(TenantModel model)
    {
        if (string.IsNullOrEmpty(model.ThirdPartyProfile))
        {
            if (string.IsNullOrEmpty(model.Provider) ||
                (string.IsNullOrEmpty(model.AccessToken) && string.IsNullOrEmpty(model.CodeOAuth)))
            {
                return (null, false);
            }
        }
        ...
        try
        {
            var profile = !string.IsNullOrEmpty(model.ThirdPartyProfile) ?
                await loginProfileTransport.FromPureTransport(model.ThirdPartyProfile) :
                providerManager.GetLoginProfile(model.Provider, model.AccessToken, model.CodeOAuth);
        ...
        LoginProfile loginProfile = null)
    {
        ...
        if (!string.IsNullOrEmpty(model.PasswordHash))
        {
            ...
            sendCongratulationsAddress = autoGeneratedEmail ? null : await commonMethods.SendCongratulations(scheme, t, model.SkipWelcome, info.Address);
