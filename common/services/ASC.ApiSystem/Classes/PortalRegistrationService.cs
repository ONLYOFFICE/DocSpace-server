    ...
    ConsumerFactory consumerFactory)
{
    ...
    public async Task<(PortalRegistrationResponseDto Response, PortalRegistrationErrorDto Error, int StatusCode)> HandleRegisterAsync(TenantModel model)
    {
        ...
        {
            Reference = reference,
            Tenant = commonMethods.ToTenantResponseDto(t),
    ...
    {
        var (loginProfile, autoGenaratedEmail) = await ProcessThirdPartyProfileAsync(model);
        ...
        var activationStatus = autoGenaratedEmail ? EmployeeActivationStatus.AutoGenerated : EmployeeActivationStatus.Activated;
        ...
        return (new PortalRegistrationResponseDto
        {
            ...
            Tenant = commonMethods.ToTenantResponseDto(t),
            ReferenceWelcome = sendCongratulationsAddress,
            AutoGenaratedEmail = autoGenaratedEmail
        }, null, StatusCodes.Status200OK);
    }
        ...
        LoginProfile loginProfile = null)
    {
        ...
        if (!string.IsNullOrEmpty(model.PasswordHash))
        {
            var autoGenaratedEmail = info.ActivationStatus == EmployeeActivationStatus.AutoGenerated;
