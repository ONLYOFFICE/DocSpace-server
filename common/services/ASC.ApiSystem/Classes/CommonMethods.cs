    ...
    IHttpClientFactory clientFactory)
{
    ...
    public async Task<string> SendCongratulations(string requestUriScheme, Tenant tenant, bool skipWelcome,  string address)
    {
        return await CallSendMethod(requestUriScheme, "portal/sendcongratulations", HttpMethod.Post, tenant, ConfirmType.Auth, skipWelcome, address);
    }
    ...
    private async Task<string> CallSendMethod(string requestUriScheme, string apiMethod, HttpMethod httpMethod, Tenant tenant, ConfirmType confirmType, bool skipAndReturnUrl,  string address = null)
    {
        ...
        if (!string.IsNullOrEmpty(address) && domain == "localhost" && coreBaseSettings.Standalone)
        {
            request.Headers.Add("Origin", $"{requestUriScheme}{Uri.SchemeDelimiter}{address}");
        }
