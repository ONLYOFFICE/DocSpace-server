    ...
    : SocketServiceClient(tariffService, tenantManager, channelWriter, machinePseudoKeys, configuration)
{
    ...
    public async Task ChangeAppEnabledAsync(string id, bool enabled)
    {
        var room = GetAppsRoom();

        await MakeRequest("change-app-enabled", new { room, id, enabled });
    }

    private string GetAppsRoom()
    {
