...
public class CachedUserService : IUserService
{
    ...
    public async Task<byte[]> GetUserPhotoAsync(int tenant, Guid id)
    {
        ...
        }, opt => opt.SetDuration(_photoExpiration).SetFailSafe(true), [CacheExtention.GetUserPhotoTag(tenant, id)]);
        return photo;
    }
    ...
    public async Task<Dictionary<Guid, UserRelation>> GetUserRelationsAsync(int tenantId, Guid sourceUserId)
    {
        ...
        }, opt => opt.SetDuration(_cacheExpiration).SetFailSafe(true), [CacheExtention.GetRelationTag(tenantId, sourceUserId)]);
    ...
    public async Task<Group> GetGroupAsync(int tenant, Guid id)
    {
        ...
        }, opt => opt.SetDuration(_cacheExpiration).SetFailSafe(true), [CacheExtention.GetGroupTag(tenant, id)]);
